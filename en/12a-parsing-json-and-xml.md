...
We've seen how to convert your type definitions to s-expressions by
using the powerful `camlp4` syntax extensions. Now we're going to move
on to parsing a common Internet formats called JSON, which is used in
many web services.  You'll also learn how to use external tools
to auto-generate OCaml type definitions here, as an alternative to using
`camlp4`.
