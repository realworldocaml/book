...
errors and other return values.  In particular, whether you consider
not finding an element in a list or hashtable to be an error depends
...
But it turns out that generating error messages is an expensive
business.  Generating an ASCII representation of a type can be quite
time-consuming, particularly if it includes expensive-to-convert
numerical datatypes.

`Error` gets around this issue by being lazy.  You can construct an
`Error` in a number of ways.  You can, for example, construct one from
a string:
...
When you construct an `Error.t` this way, the thunk is not actually
executed until someone decides to serialize the `Error.t` to a string.
In the toplevel, this happens immediately when the `Error.t` is
printed out.  But in a compiled program, it's not unusual for most
`Error.t`'s that are generated by the program to be ignored and never
serialized before.  In this case, the laziness of `Error.t` is a big
win.

Another convenient way to construct an `Error.t` is using Core's
s-expression converters.  Thus, we can write:
...
s-expression, isn't run until the error is serialized.  Using the
Sexplib syntax-extension, which is discussed in more detail in chapter
{{SYNTAX}}, we can include multiple pieces of data in an `Error.t`:
...
Sexplib to generate the sexp-converter for the type in question.
