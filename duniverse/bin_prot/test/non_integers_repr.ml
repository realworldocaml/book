...
module Mat = struct
  ...
  let to_array t =
    Array.init (Array2.dim1 t) ~f:(fun i ->
      Array.init (Array2.dim2 t) ~f:(fun j ->
        t.{i+1,j+1}))
...
module Tests = struct
  ...
  let float =
    ...
    ; values   = [ Float.epsilon_float; Float.infinity; Float.max_finite_value
                 ; Float.max_value ; Float.min_positive_normal_value
                 ; Float.min_positive_subnormal_value; Float.min_value; Float.minus_one
                 ; Float.neg_infinity; Float.one
                 ; Float.robust_comparison_tolerance; Float.zero
                 ]
  ...
  let bigstring =
    ...
    ; equal    = (fun s1 s2 ->
        String.equal (Bigstring.to_string s1) (Bigstring.to_string s2))
  ...
  let lazy_t =
    ...
    ; values   = [ lazy 0l; lazy 1l; lazy (-1l); lazy Int32.max_value; lazy Int32.min_value ]
  ...
  let option =
    ...
    ; values   = [ None; Some 0l; Some 1l; Some (-1l); Some Int32.max_value
  ...
  let pair =
    ...
    ; values   = [ 0l, 0l; 1l, 1l; -1l, -1l; Int32.max_value, Int32.max_value
  ...
  let triple =
    { writer   = Write.bin_write_triple
                   Write.bin_write_int32 Write.bin_write_int32 Write.bin_write_int32
    ; reader   = Read .bin_read_triple
                   Read .bin_read_int32  Read .bin_read_int32  Read .bin_read_int32
    ; values   = [ 0l, 0l, 0l; 1l, 1l, 1l; -1l, -1l, -1l
  ...
  module R1 = struct
    type t = { x : Int32.t; y : float } [@@deriving bin_io, fields, sexp_of]
  ...
  let record1 =
    ...
    ; reader   = bin_read_t
    ; values   = [ { x = 0l; y = 0.0 }
                 ; { x = Int32.max_value; y = Float.max_value }
                 ]
    ; equal    = ( = )
    ; sexp_of  = [%sexp_of: t]
...
let gen_tests t =
  ...
  in
  let hex_size =
    List.fold bin_protted_values ~init:0 ~f:(fun acc s ->
      Int.max acc (String.length s))
  in
  let min, max =
    List.fold2_exn t.values bin_protted_values ~init:(Int.max_value, 0)
      ~f:(fun (min, max) v s ->
        ...
        if len < t.lo_bound || len > hi_bound then
          printf ", bin_size outside of range %d..%d: %d"
            t.lo_bound hi_bound len;
        if not (t.equal v v') || len <> len' then
          printf !", read test failed: read %d byte%s as %{Sexp}" len'
            (if len' = 1 then "" else "s") (t.sexp_of v');
        Out_channel.output_char stdout '\n';
        (Int.min min len, Int.max max len))
  in
  match t.hi_bound with
  | None ->
    if min <> t.lo_bound then
      printf "invalid lower bound: %d, expected: %d\n"
        min t.lo_bound
  | Some hi_bound ->
    if min <> t.lo_bound || max <> hi_bound then
      printf "invalid bounds: %d..%d, expected: %d..%d\n"
