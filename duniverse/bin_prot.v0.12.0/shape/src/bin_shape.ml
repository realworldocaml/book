...
module Sorted_table : sig
  ...
  type 'a t = {
    sorted : (string * 'a) list
  }
  [@@deriving compare, sexp]

  let merge_check_adjacent_dups : eq:('a -> 'a -> bool) -> (string * 'a) list ->
      ...
      let rec loop acc ~last_key ~last_value = function
        ...
        | (key,value)::xs ->
          ...
          then
            ...
            loop ((key,value)::acc) ~last_key:key ~last_value:value xs
      in
      function
      | [] -> `Ok []
...
module Canonical_exp_constructor = struct
  ...
  let to_string t =
    Sexp.to_string (sexp_of_t (fun _ -> Atom "...") t)

end

module Create_digest : sig (* Digest various expression forms *)
  ...
  let digest_layer = function
    ...
    | Base (u, l) ->
      Digest.constructor "base" [Digest.uuid u; Digest.list l]
    | Tuple l ->
      Digest.constructor "tuple" [Digest.list l]
    | Record l ->
      Digest.constructor "record" [Digest.list (List.map l ~f:(fun (s, t) ->
        Digest.pair (Digest.string s) t
      ))]
    | Variant l ->
      Digest.constructor "variant" [Digest.list (List.map l ~f:(fun (s, l) ->
        Digest.pair (Digest.string s) (Digest.list l)
      ))]
    | Poly_variant table ->
      Digest.constructor "poly_variant"
        [ Digest.list (
            List.map (Sorted_table.expose table)
              ~f:(fun (x,y) ->
                Digest.pair
                  (Digest.string x)
                  (Digest.option y)
    ...
    | Application (x, l) ->
      Digest.constructor "application"
        [ x;
          Digest.list l
        ]
    | Rec_app (n, l) ->
      Digest.constructor "rec_app"
        [ Digest.int n;
          Digest.list l
        ]
    | Var n ->
      Digest.constructor "var" [ Digest.int n ]
...
module Canonical_digest : Canonical = struct
  ...
  module Exp1 = struct
    ...
    let to_digest (type a) (x : a t) = match x with
      | Opaque x -> x
    ...
    let get_poly_variant (x : Visibility.visible t) = match x with
      | Non_poly_variant (desc,_) -> Error desc
      | Poly_variant l ->
        Ok (Sorted_table.map ~f:(Option.map ~f:(fun x -> Opaque x)) l)
  end
...
module Canonical_full : sig
  ...
  module Exp1 = struct
    ...
    let poly_variant loc xs =
      Exp (Canonical_exp_constructor.Poly_variant (Sorted_table.create loc ~eq:(equal_option equal_t0) xs))
...
module Gid : sig (* unique group-id, used as key for Tenv below *)
  type t  [@@deriving compare, sexp_of]
  val create : unit -> t
end= struct
  type t = int [@@deriving compare, sexp_of]
  let r = ref 0
  let create () = let u = !r in r:=1+u; u
end

module Expression = struct

  type 't poly_constr = [`Constr of string * 't option | `Inherit of Location.t * 't ]
  ...
  module Group : sig
    ...
    let lookup g tid =
      ...
      | None ->
        eval_fail g.loc
          !"impossible: lookup_group, unbound type-identifier: %{Tid}" tid ()
  ...
  let is_cyclic_0 ~(via_VR:bool) : group -> Tid.t -> bool =
    fun group tid ->
      ...
      let rec trav = function
        ...
        | Variant xs -> if via_VR then List.iter xs ~f:(fun (_,ts) -> List.iter ~f:trav ts) else ()
        (* We dont follow type-vars *)
        | Var _
          -> ()
        (* traverse (recursive) type-apps when first encountered *)
        | Rec_app (tid,ts)
          -> (if visited tid then () else (add tid; trav_tid tid)); List.iter ts ~f:trav
      and trav_tid tid =
  ...
  *)
    ...
    fun group venv t ->
      ...
      | Record binds ->
        sequence_defining binds ~f:(fun (s,x) ->
          eval group venv x >>= fun y ->
          return (s,y)
        ) >>= fun binds ->
        return (Canonical.Create.record binds)
      | Variant alts ->
        sequence_defining alts ~f:(fun (s,xs) ->
          eval_list group venv xs >>= fun ys ->
          return (s,ys)
        ) >>= fun alts ->
        return (Canonical.Create.variant alts)
      | Var (loc,vid) ->
        begin match Venv.lookup venv vid with
        | Some x -> return x
        | None -> eval_fail loc !"Free type variable: '%{Vid}" vid ()
        end
      | Annotate (s,t) ->
        eval group venv t >>= fun v ->
        return (Canonical.Create.annotate s v)
      | Base (s,ts) ->
        eval_list group venv ts >>= fun vs ->
        return (Canonical.Create.basetype s vs)
      | Tuple ts ->
        eval_list group venv ts >>= fun vs ->
        return (Canonical.Create.tuple vs)
      | Top_app (in_group,tid,args) ->
        eval_list group venv args >>= fun args -> (* args evaluated in current group *)
        eval_app in_group tid args (* group changed here *)
      | Rec_app (tid,args) ->
        eval_list group venv args >>= fun args ->
        eval_app group tid args
      | Poly_variant (loc,cs) ->
        sequence_defining ~f:(eval_poly_constr group venv) cs >>= fun xss ->
        return (Canonical.Create.poly_variant loc (List.concat xss))

  and eval_list : group -> Venv.t -> t list ->
    _ Canonical.Exp1.t list defining =
    fun group venv ts ->
      sequence_defining ts ~f:(eval group venv)

  and eval_poly_constr : group -> Venv.t -> t poly_constr
    ...
    fun group venv c ->
      ...
      | `Constr (s,Some t) ->
        eval group venv t >>= fun v ->
        return [(s,Some (Canonical.Exp1.opaque v))]
      | `Inherit (loc,t) ->
        eval group venv t >>= fun v ->
        begin match Canonical.Exp1.get_poly_variant v with
  ...
  and eval_definition : group -> Vid.t list -> t -> Canonical.Def.t defining =
    fun group formals body ->
      let venv = Venv.create (List.mapi formals ~f:(fun i x ->
        x, Canonical.Exp1.var i
      )) in
      eval group venv body >>= fun v ->
      return (Canonical.Create.define v)
  ...
  and eval_app : group -> Tid.t -> _ Canonical.Exp1.t list -> _ Canonical.Exp1.t defining =
    fun group tid args ->
      ...
      let record_or_normal_variant =
        ...
        | (Tuple _ | Annotate _ | Base _ | Poly_variant _ | Var _ | Rec_app _ | Top_app _ ) -> false
      in
      ...
      then (
        begin
          Defining.look_env (gid,tid) >>= function
          | Some recurse -> return recurse
          | None ->
            Defining.extend_new_tid (gid,tid) (
              eval_definition group formals body
            )
        end >>= function
...
let eval_to_digest exp =
  Canonical_selected.to_digest (Evaluation_to_digest.eval exp)

let eval_to_digest_string exp =
  Digest.to_hex (eval_to_digest exp)
