...
module Testable = struct
  let parse_parts_part_decl =
    ...
    let pp fmt =
      ...
      | Compat_attr (n, sep_indent) -> Fmt.pf fmt "Compat_attr (%S, %S)" n sep_indent
      | Part_begin (n, sep_indent) -> Fmt.pf fmt "Part_begin (%S, %S)" n sep_indent
...
let test_of_line =
  let make_test ~line ~expected () =
    let test_name = Printf.sprintf "parse_line: %S" (match line with
      | Ok s -> s
