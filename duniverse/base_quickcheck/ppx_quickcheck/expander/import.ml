...
let gensyms prefix loc_list =
  List.map loc_list ~f:(gensym prefix)
  |> List.unzip

let gensymss prefix loc_list_list =
  List.map loc_list_list ~f:(gensyms prefix)
  |> List.unzip
...
let fn_map_label ~loc ~from ~to_ =
  ...
  pexp_fun ~loc Nolabel None f_pat
    (pexp_fun ~loc to_ None x_pat
       (pexp_apply ~loc f_expr
          [(from, x_expr)]))
