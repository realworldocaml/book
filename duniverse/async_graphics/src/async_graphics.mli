...
(** Default background and foreground colors (usually, either black
   ...
   black background).
   {!Async_graphics.clear_graph} fills the screen with the [background] color.
   The initial drawing color is [foreground]. *)


(** {2 Some predefined colors} *)
...
(** See {!Async_graphics.draw_string}.*)
...
(** Fill an elliptical pie slice with the current color. The
   parameters are the same as for {!Async_graphics.draw_arc}. *)

val fill_ellipse : int -> int -> int -> int -> unit
(** Fill an ellipse with the current color. The
   parameters are the same as for {!Async_graphics.draw_ellipse}. *)

val fill_circle : int -> int -> int -> unit
(** Fill a circle with the current color. The
   parameters are the same as for {!Async_graphics.draw_circle}. *)
...
(** Capture the contents of a rectangle on the screen as an image.
   The parameters are the same as for {!Async_graphics.fill_rect}. *)
...
(** Set display mode on or off. When turned on, drawings are done
   ...
   drawing into the backing store (see the function {!Async_graphics.remember_mode}
...
(** Set remember mode on or off. When turned on, drawings are done
   ...
   onto the graphics window (see the function {!Async_graphics.display_mode} above).
