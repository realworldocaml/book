...
let arrow
  ...
  let input_generator =
    ...
    | Nolabel | Labelled _ ->
      generator_of_core_type input_type
    | Optional _ ->
      [%expr
...
let compound_hash ~loc ~size_expr ~hash_expr ~hash_pat ~observer_exprs ~field_exprs =
  ...
  List.fold_right alist
    ~init:hash_expr
    ~f:(fun (observer_expr, field_expr) body_expr ->
      [%expr
        let [%p hash_pat] =
...
let compound
  ...
  let observer_exprs =
    List.map fields ~f:(fun field ->
      observer_of_core_type (Field.core_type field))
  in
         ...
         [%e
           pexp_match ~loc expr
             (List.map clauses ~f:(fun clause ->
                ...
                let observer_exprs =
                  List.map core_type_list ~f:observer_of_core_type
                in
                let field_pats, field_exprs =
                  gensyms "x" (List.map core_type_list ~f:(fun core_type ->
                    core_type.ptyp_loc))
                in
