...
let compound_sequence ~loc ~make_compound_expr ~field_pats ~field_exprs ~shrinker_exprs =
  [%expr
    Base.Sequence.round_robin
      [%e
        elist ~loc
          (List.map3_exn field_pats field_exprs shrinker_exprs
...
let compound
  ...
  let shrinker_exprs =
    List.map fields ~f:(fun field ->
      shrinker_of_core_type (Field.core_type field))
  in
  [%expr
        ...
        pexp_function ~loc
          (List.map clauses ~f:(fun clause ->
             ...
             let field_pats, field_exprs =
               gensyms "x"
                 (List.map core_type_list ~f:(fun core_type ->
                    core_type.ptyp_loc))
             in
             let shrinker_exprs =
               List.map core_type_list ~f:shrinker_of_core_type
             in
