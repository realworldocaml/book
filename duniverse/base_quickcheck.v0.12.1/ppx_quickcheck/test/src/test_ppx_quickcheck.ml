...
module Helpers = struct
  ...
  let m_all (type a) (module M : All with type t = a)
    ...
    = (module struct include M let examples = all end)
end
open Helpers
...
let%expect_test "record type" =
  let module Record_type' = struct
    type t = Record_type.t = { x : bool; y : unit option }
    [@@deriving compare, enumerate, sexp_of]
  end in
...
let%expect_test "first order arrow type" =
  ...
  let test ?cr ?(config = config) m =
    test ?cr ~config ~shrinker:`atomic m
  in
...
let%expect_test "polymorphic type" =
  ...
  [%expect {|
    ...
    (observer transparent)
    (shrinker (((false (())) => (false ())) ((true (())) => (true ())))) |}];
  test (module Instance_with_variance)
...
let%expect_test "recursive type" =
  let module Recursive' = struct
    type t = Recursive.t = Leaf | Node of t * t [@@deriving compare, hash, sexp_of]

    let rec create = function
      | 0 -> Leaf
      | n -> let t = create (n - 1) in Node (t, t)

    let examples = List.init 3 ~f:create
  end in
  test (module Recursive) (module Recursive');
  [%expect {|
    ...
    (shrinker
     (((Node Leaf Leaf) => Leaf)
      ((Node Leaf Leaf) => Leaf)
      ((Node (Node Leaf Leaf) (Node Leaf Leaf)) => (Node Leaf Leaf))
      ((Node (Node Leaf Leaf) (Node Leaf Leaf)) => (Node Leaf Leaf))
      ((Node (Node Leaf Leaf) (Node Leaf Leaf)) => (Node Leaf (Node Leaf Leaf)))
      ((Node (Node Leaf Leaf) (Node Leaf Leaf)) => (Node (Node Leaf Leaf) Leaf))
      ((Node (Node Leaf Leaf) (Node Leaf Leaf)) => (Node Leaf (Node Leaf Leaf)))
      ((Node (Node Leaf Leaf) (Node Leaf Leaf)) => (Node (Node Leaf Leaf) Leaf)))) |}];
;;
