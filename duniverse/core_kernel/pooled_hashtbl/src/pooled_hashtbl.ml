...
module Make_plain (Key : Key_plain) = struct
  ...
  module Provide_of_sexp(X : sig type t [@@deriving of_sexp] end with type t := key) =
  struct
    let t_of_sexp v_of_sexp sexp = t_of_sexp X.t_of_sexp v_of_sexp sexp
  end

  module Provide_bin_io(X : sig type t [@@deriving bin_io] end with type t := key) =
    Bin_prot.Utils.Make_iterable_binable1 (struct
      module Key = struct include Key include X end
      type nonrec 'a t = 'a t
