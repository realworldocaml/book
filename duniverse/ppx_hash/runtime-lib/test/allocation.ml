...
let check_allocation : expect:int -> (unit -> 'a) -> 'a = fun ~expect f ->
  ...
  if n <> expect then (
    failwithf "check_allocation: expect=%d, got=%d"
      expect n ()
  );
  res
...
module F(Hash : Base.Hash.S) = struct
  ...
  type tree = Leaf of int | Node of tree * tree [@@deriving sexp_of, hash]

  let create_full ~depth =
    assert (depth>=0);
    let rec build n d =
      if d = 0 then n+1, Leaf n else
        let n,t1 = build n (d-1) in
        let n,t2 = build n (d-1) in
        n,Node(t1,t2)
    in
    let _,t = build 100 depth
    in t
...
module Test_alloc (X :sig
  ...
  let%test_unit _ =
    let state =
      check_allocation ~expect:X.size_of_state (fun () ->
        Hash.alloc ()
      )
    in
    let _state =
      check_no_allocation (fun () ->
        hash_fold_tree state the_tree
      )
    in
    ()
  ...
  let%test_unit _ =
    assert (not (run_seeded X.seed1 = run_seeded X.seed2))

  let%test_unit _ =
    ignore (check_allocation ~expect:X.size_of_state (fun () ->
      hash_tree the_tree
    ))
  ...
  let%test_unit _ =
    ...
    ignore (check_allocation ~expect:(n * X.size_of_state) (fun () ->
  ...
  let%test_unit "hashing is stable2" =
    [%test_eq: int] res1 res2
