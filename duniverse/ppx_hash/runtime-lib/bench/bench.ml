...
module Bench (Hash : Hash.S) = struct
  ...
  let%bench_module "" [@name_suffix Hash.description] =
    (module struct
      ...
      type c = Foo | Bar | Baz of a * b * c     [@@deriving hash]
      ...
      let c2 = Baz (a, b, c1)
      let rec cn n =
        if n <= 1 then c1 else Baz (n, (n, float_of_int n), cn (n - 1))

      let c10 = cn 10
      let c100 = cn 100;;

      let rec dn n =
        if n <= 0 then [] else (n, String.init n ~f:(fun i -> if i mod 2 = 0 then 'j' else 's')) :: dn (n -1)
...
module Check_initialized_correctly : Hash.S = struct
  ...
  type hash_value = int
  type state = { me : int; valid : int ref; }
  type seed = unit

  let next_id = let x = ref 0 in (fun () -> incr x; !x)
