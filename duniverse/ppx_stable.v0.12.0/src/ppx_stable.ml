...
module Invariant = struct
  ...
  let things_are_known ~loc ~all ~thing_name ~supposed_to_be to_remove =
    ...
    if not (Set.is_empty unknown_fields) then (
      let str = set_to_string unknown_fields in
      Location.raise_errorf ~loc
        "Some %s were supposed to be %s but they were not found: %s" thing_name supposed_to_be str
    );
end
...
(* fun ~name:name -> exp *)
let mk_pexp_fun ~loc ~name exp =
  pexp_fun ~loc (Labelled name) None (pvar ~loc name) exp
...
let convert_record ~loc ~source_fields ~target_fields ~modified_fields ~set_fields
  ...
  let record_pat =
    let record_pat =
      List.map (Set.to_list source_fields) ~f:(fun name ->
        if Set.mem target_fields name && not (Set.mem set_fields name) then
          (mk_lident ~loc name, ppat_var ~loc (Located.mk ~loc name))
        else
          (mk_lident ~loc name, ppat_any ~loc)
      )
    in
  ...
  let rhs_record =
    let rhs_fields =
      List.map (Set.to_list target_fields) ~f:(fun name ->
        if Set.mem modified_fields name then (
          let f = evar ~loc (modify_field_name name) in
          let application =
            pexp_apply ~loc f [Asttypes.Nolabel, evar ~loc name]
          in
  ...
  in
  let acc =
    [%expr ([%e rhs_record] : [%t target_type])]
  in
  let acc =
    Set.fold (Set.union set_fields (Set.diff target_fields source_fields))
      ~init:acc ~f:(fun acc name -> mk_pexp_fun ~loc ~name acc)
  in
  let acc =
...
let generate_stable_variant_module ~td ~loc ~cdl =
  let alias_args cd =
    ...
    | Pcstr_record lds ->
      List.mapi lds ~f:(fun i _ -> "v" ^ Int.to_string i)
    | Pcstr_tuple fields ->
      List.mapi fields ~f:(fun i _ -> "v" ^ Int.to_string i)
  in
  let alias_fun_label name = name ^ "_fun" in
  let labels_and_aliases cd =
    match cd.pcd_args with
    | Pcstr_tuple _ ->
      List.map (alias_args cd) ~f:(fun alias -> Nolabel, alias)
    | Pcstr_record lds ->
      ...
      let expr =
        let constructed_value =
          let arg =
            match cd.pcd_args with
            | Pcstr_tuple _ ->
              pexp_tuple_opt ~loc (List.map arg_aliases ~f:(fun alias -> evar ~loc alias))
            | Pcstr_record lds ->
              Some (
                pexp_record ~loc
        ...
        in
        if List.is_empty arg_aliases then
          [%expr fun () -> [%e constructed_value]]
        else (
          List.fold_right (labels_and_aliases cd)
  ...
  let map_function =
      ...
      List.map cdl ~f:(fun cd ->
        ...
        let pattern =
          let arg =
            match cd.pcd_args with
            | Pcstr_tuple _ ->
              ppat_tuple_opt ~loc (List.map arg_aliases ~f:(fun alias -> pvar ~loc alias))
            | Pcstr_record lds ->
              Some (
                ppat_record ~loc
        ...
        let value =
          ...
          if List.is_empty arg_aliases then
            ...
            pexp_apply ~loc fun_expr
              (List.map (labels_and_aliases cd)
                 ~f:(fun (lbl, alias) -> lbl, evar ~loc alias))
          )
        in
    ...
    in
    mk_module ~loc ~name:"Helper" ~items:[
...
let convert_variant ~loc ~source_variants ~target_variants ~modified_variants ~target_type
      ~source_type =
  ...
  in
  let acc =
    [%expr [%e (variants_longident ~loc ~which_type:source_type
                  [stable_variants ~type_:source_type; "Helper"; "map"])] v]
  in
  let rhs =
    Set.fold source_variants ~init:acc ~f:(fun acc name ->
      let f =
        if Set.mem modified_variants name then
          evar ~loc (modify_field_name name)
        else if not (Set.mem target_variants name) then
          evar ~loc (remove_field_name name)
        else (
          variants_longident ~loc ~which_type:target_type
            [ stable_variants ~type_:target_type
...
let conversions_of_td ~ppx_name ~target_type
      ?(add=[]) ?(remove=[]) ?(modify=[]) ?(set=[]) td =
    ...
    | Some target_type ->
      ...
      let to_target, of_target =
        ...
        | Ptype_record lds ->
          ...
          in
          Invariant.things_are_known ~thing_name:"fields" ~supposed_to_be:"removed"
            ~loc ~all:current_fields remove;
          Invariant.things_are_known ~thing_name:"fields" ~supposed_to_be:"modified"
            ~loc ~all:current_fields modify;
          Invariant.things_are_known ~thing_name:"fields" ~supposed_to_be:"set"
            ~loc ~all:current_fields set;
        ...
        | Ptype_variant cdl ->
          ...
          in
          Invariant.things_are_known ~thing_name:"variants" ~supposed_to_be:"removed"
            ~loc ~all:current_variants remove;
          Invariant.things_are_known ~thing_name:"variants" ~supposed_to_be:"modified"
            ~loc ~all:current_variants modify;
          (if not (Set.is_empty set) then
             Location.raise_errorf ~loc "[set] is for record only");
...
let gen ppx_name ~loc ~path:_ (_rec, tds) target_type add modify set remove =
  ...
  | _ -> Location.raise_errorf ~loc "mutually recursive types are not \
                                     supported by ppx_stable_type"
...
let stable_variant =
  ...
  let str_type_decl =
    Deriving.Generator.make args (gen name) ~deps:[]
  in
  Deriving.add name ~str_type_decl
