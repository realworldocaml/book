...
let sexp_of_t :
  ...
  fun _of_a ->
    function
    | { num_mutations = v_num_mutations; front = v_front; mask = v_mask;
      ...
      let bnds =
        ...
        (Ppx_sexp_conv_lib.Sexp.List
           [Ppx_sexp_conv_lib.Sexp.Atom "mask"; arg])
        :: bnds in
      let bnds =
        let arg = sexp_of_int v_front in
        (Ppx_sexp_conv_lib.Sexp.List
           [Ppx_sexp_conv_lib.Sexp.Atom "front"; arg])
        :: bnds in
      let bnds =
        let arg = sexp_of_int v_num_mutations in
        (Ppx_sexp_conv_lib.Sexp.List
           [Ppx_sexp_conv_lib.Sexp.Atom "num_mutations"; arg])
        :: bnds in
...
let check_index_exn t i =
  if i < 0 || i >= t.length
  then Error.raise_s
         (Sexp.message "Queue index out of bounds"
            [ "index" , i        |> Int.sexp_of_t
            ; "length", t.length |> Int.sexp_of_t ])
;;
...
let ensure_no_mutation t num_mutations =
  ...
  then Error.raise_s
         (Sexp.message "mutation of queue during iteration"
...
let create (type a) ?capacity () : a t =
  let capacity =
    ...
    | Some capacity ->
      if capacity < 0
      then Error.raise_s
             (Sexp.message "cannot have queue with negative capacity"
...
let peek t =
  if is_empty t
  then None
  else Some (front_nonempty t)
;;

let peek_exn t =
  if is_empty t
  then raise Caml.Queue.Empty
  else front_nonempty t
;;
...
let blit_transfer ~src ~dst ?len () =
  ...
  let len =
    ...
    | Some len ->
      if len < 0
      then Error.raise_s
             (Sexp.message "Queue.blit_transfer: negative length"
  ...
  if len > 0 then begin
    ...
    for i = 0 to len - 1 do
      ...
      Option_array.unsafe_set_some dst.elts dst_i
...
let init len ~f =
  if len < 0
  then Error.raise_s
         (Sexp.message "Queue.init: negative length"
            [ "length", len |> Int.sexp_of_t ]);
