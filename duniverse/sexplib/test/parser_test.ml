...
let sexp_of_layout_sexps_or_something sexps_with_layout =
  ...
  | ([] | _ :: _ :: _) as sexps ->
    ...
    Sexp.List [ Sexp.List sexps; Sexp.List (sexps_of_layout_sexps sexps_with_layout) ]
;;
...
let parsers =
  ...
  ; ( (fun s ->
        ...
        | Sexp.Cont (cont_state, parse_fun) ->
          let rec aux parse_fun pos =
            ...
            | Sexp.Cont (cont_state, parse_fun) -> aux_cont pos cont_state parse_fun
          and aux_cont pos cont_state parse_fun =
            if pos + 1 = String.length s
            then (
              match cont_state with
              | Sexp.Cont_state.Parsing_atom ->
              ...
              | Sexp.Cont_state.Parsing_nested_whitespace -> failwith "incomplete"
              | Sexp.Cont_state.Parsing_list -> failwith "Should have reached the end3")
            else aux parse_fun (pos + 1)
          in
  ...
  ; ( (fun s ->
        let parse =
          ...
          fun () -> Sexp.With_layout.Parser.sexp_opt Sexp.With_layout.Lexer.main lexbuf
        in
        let rec aux acc =
          match parse () with
          | None -> List.rev acc
...
let list_parsers =
  ...
  ; ( (fun s -> Sexp.List (Sexp.scan_sexps_conv ~f:(fun x -> x) (Lexing.from_string s)))
    , "ocamllex_opt" )
  ; ( (fun s ->
        Sexp.List
...
let%test_unit _ =
  ...
  (* single line comment at toplevel *)
  same_parse_tree [%here] ";;\nb" "b";
  (* single line comment don't nest *)
  same_parse_tree [%here] ";\"\nb" "b";
  (* single line comment ignore quotes *)
  same_parse_tree [%here] "(a#)" "(a#;\n)";
  parse_fail [%here] "a#|" (function
  ...
  parse_fail_trees ~no_following_sibling:true [%here] "\"\\\n" (function
    | Failure s ->
      ...
      || grep "reached EOF while in state" s
    | _ -> false);
  parse_fail_trees ~no_following_sibling:true [%here] "\"\\\013" (function
    | Failure s ->
      grep "incomplete" s
      || grep "unterminated" s
      || grep "empty token" s
  ...
  parse_fail [%here] "\"hello" (* unterminated quoted atom *) (function
    ...
    | _ -> false);
  parse_fail [%here] "\013x" (* weird newline *) (function
    | Failure s -> grep "unexpected character" s || grep "empty token" s
    | Sexp.Parse_error _ -> true
    | _ -> false);
  parse_fail_trees [%here] "\013" (* trailing weird newline *) (function
    | Failure s ->
      grep "incomplete" s
      || grep "empty token" s
      || grep "reached EOF while in state" s
    ...
    | Sexp.Parse_error _ -> true
    | _ -> false);
  parse_fail [%here] "|#" (* not started *) (function
    | Failure s -> grep "illegal end of comment" s
    | Sexp.Parse_error _ -> true
    | _ -> false);
  parse_fail [%here] ~no_following_sibling:true "#;" (* not followed *) (function
    | Sexp.Parse_error _ | Failure _ -> true
    | _ -> false);
  same_parse_tree [%here] "#;a b" "b";
  (* sexp comment + atom *)
  same_parse_tree [%here] "#;((a)) b" "b";
  (* sexp comment + list *)
  same_parse_tree [%here] "#;\"#;\" b" "b";
  (* sexp comment + quoted atom *)
  same_parse_tree [%here] "#;a;comment\nb" "b";
  (* sexp comment + single line commment *)
  same_parse_tree [%here] "#;#|aa)|#comment b" "b";
  (* sexp comment + block commment *)
  same_parse_tree [%here] "#;a #;(a) #;\"asd\" b" "b";
  (* consecutive sexp comment + atom *)
  same_parse_tree [%here] "(#;a #;(a) #;b)" "()";
  (* consecutive sexp comment + nothing *)
  same_parse_tree [%here] "#; #; #; comment1 comment2 comment3 a" "a";
  (* nested sexp comment *)
  same_parse_tree [%here] "#| ; |# ()" "()";
  ...
  same_parse_trees [%here] "#;#;a b c d " "c d ";
  (* leading comments work alright *)
  same_parse_trees [%here] "#;#;a b " " ";
  (* leading comments work alright *)
  same_parse_trees [%here] "plop #;#;a b " "plop ";
  (* trailing comments work alright *)
