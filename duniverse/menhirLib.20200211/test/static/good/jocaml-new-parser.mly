...
/* $Id: new_parser.mly,v 1.1 1999/09/30 13:09:13 lefessan Exp $ */
...
expr:
  ...
  | LET rec_flag let_bindings error %prec prec_let
      { unclosed "let" 1 "in" 4 }
  | LBRACEBAR process BARRBRACE /* Join */
      { mkexp(Pexp_process $2) }
  | simple_expr LESSLESS list_of_exprs GREATERGREATER /* Join */
      { mkexp(Pexp_call($1, List.rev $3 )) }
  | simple_expr LESSLESS list_of_exprs error /* Join */
      { unclosed "<<"  2 ">>" 4 }
  | simple_expr LESSLESSGREATERGREATER /* Join */
      { mkexp(Pexp_call($1, [] )) }
...
location_tail:
    /* empty */                               { void () }
  | SEMISEMI                                  { void () }
  | SEMISEMI location_item                    { $2 }
  | location_item location_tail               { $1 }
;
...
static_semi_list:
    static_expr %prec prec_list                           { [$1] }
  | static_semi_list SEMI static_expr %prec prec_list    { $3 :: $1 }
;
static_expr:
    val_longident
      { mkexp(Pexp_ident $1) }
...
static_comma_list:
    static_expr                           { [$1] }
  |  static_comma_list COMMA static_expr   { $3 :: $1 }
;
/* END Join */
...
core_type:
    simple_core_type
      { $1 }
  | LESSLESS channel_type GREATERGREATER /* Join */
      { mktyp(Ptyp_channel(List.rev $2,Pasync)) }
  | LESSLESS channel_type EQUALGREATER core_type GREATERGREATER  /* Join */
...
channel_tuple:
   core_type  { [$1] }
|  channel_tuple COMMA core_type { $3 :: $1 }
;
channel_type:
       { [] }
  | channel_tuple { $1 }
