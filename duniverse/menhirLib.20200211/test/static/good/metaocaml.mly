...
/* $Id: parser.mly,v 1.20 2005/06/25 12:42:14 ccris Exp $ */
...
let mkatyp d =                                (* XOO *)
  { atyp_desc = d; atyp_loc = symbol_rloc() } (* XOO *)
let mktyp d =
  { ptyp_desc = d; ptyp_loc = symbol_rloc() }
let mkpat d =
  { ppat_desc = d; ppat_loc = symbol_rloc(); ppat_ext = None } (* XXO *)
let mkexp d =
  { pexp_desc = d; pexp_loc = symbol_rloc(); pexp_ext = None } (* XXO *)
let mkmty d =
...
let mkoperator name pos =
  { pexp_desc = Pexp_ident(Lident name); pexp_loc = rhs_loc pos; pexp_ext = None } (* XXO *)
...
(*
  ...
  it must be ghost.
*)
let ghexp d = { pexp_desc = d; pexp_loc = symbol_gloc (); pexp_ext = None };; (* XXO *)
let ghpat d = { ppat_desc = d; ppat_loc = symbol_gloc (); ppat_ext = None };; (* XXO *)
let ghtyp d = { ptyp_desc = d; ptyp_loc = symbol_gloc ()};;

let mkassert e =
  match e with
  | {pexp_desc = Pexp_construct (Lident "false", None, false,_) } -> (* XXO *)
...
%token DOTLESS     /* XXO */
%token GREATERDOT  /* XXO */
%token DOTTILDE    /* XXO */
%token DOTBANG     /* XXO */
%token DOTPOND     /* XXO */
%token DOTBANGC    /* YYO */
%token DOTBANGF    /* YYO */
%token BANGLBRACE  /* YYO */
%token RUN_GCC     /* YYO */
%token RUN_ICC     /* YYO */
%token RUN_F90     /* YYO */
%token AMPERAMPER
...
%token TYPE
%token <string> UIDENT
%token <string> EUIDENT  /* XXO */
%token <string> UIDENTI  /* XXO */
%token <string> EUIDENTI /* XXO */
%token UNDERSCORE
%token VAL
%token VIRTUAL
%token WHEN
...
expr:
  ...
  | destr_longident simple_expr %prec below_SHARP       /* XXO */
      { mkexp(Pexp_destruct($1, $2, Keep)) }            /* XXO */
  | edestr_longident simple_expr  %prec below_SHARP     /* XXO */
    {mkexp (Pexp_destruct($1,$2,Erase))}                /* XXO */
  | constr_longident simple_expr %prec below_SHARP
      { mkexp(Pexp_construct($1, Some $2, false,Keep)) }
  | econstr_longident simple_expr %prec below_SHARP
      { mkexp(Pexp_construct($1, Some $2, false,Erase)) }
...
simple_expr:
  ...
  | constr_longident %prec prec_constant_constructor
      { mkexp(Pexp_construct($1, None, false,Keep)) }
  | econstr_longident %prec prec_constant_constructor
      { mkexp(Pexp_construct($1, None, false,Erase)) }
  | name_tag %prec prec_constant_constructor
      { mkexp(Pexp_variant($1, None)) }
  | LPAREN seq_expr RPAREN
      { reloc_exp $2 }
  | DOTLESS expr GREATERDOT                 /* XXO */
      { Clflags.plain_ocaml := false; mkexp(Pexp_bracket $2) }            /* XXO */
  | DOTTILDE simple_expr %prec prec_escape  /* XXO */
      { Clflags.plain_ocaml := false; mkexp(Pexp_escape $2) }             /* XXO */
  | DOTBANG expr %prec prec_escape            /* XXO */
      { Clflags.plain_ocaml := false; mkexp(Pexp_run $2) }                /* XXO */
  | DOTPOND LBRACE core_type RBRACE  expr %prec prec_pond  /* XXO */
      { Clflags.plain_ocaml := false; mkexp(Pexp_etag ($3, $5)) }		        /* XXO */
  | DOTBANGC expr %prec prec_escape           /* YYO */
      { mkexp(Pexp_runoffshore($2, "C")) }  /* YYO */
  | DOTBANGF expr %prec prec_escape           /* YYO */
      { mkexp(Pexp_runoffshore($2, "F")) }  /* YYO */
  | BANGLBRACE record_expr RBRACE expr %prec prec_escape    /* YYO */
    {  let (exten, fields) = $2 in  mkoffShore_tree exten fields $4 }
  | BANGLBRACE record_expr_gcc RBRACE expr %prec prec_escape    /* YYO */
    {  let (exten, fields) = $2 in mkoffShore_tree exten fields $4 }
  | BANGLBRACE record_expr_icc RBRACE expr %prec prec_escape    /* YYO */
    {  let (exten, fields) = $2 in mkoffShore_tree exten fields $4 }
  | BANGLBRACE record_expr_f90 RBRACE expr %prec prec_escape    /* YYO */
    {  let (exten, fields) = $2 in mkoffShore_tree exten fields $4 }
  | BANGLBRACE record_expr error            /* YYO */
    { unclosed "{" 1 "}" 5 }                /* YYO */
  | BANGLBRACE record_expr_icc error            /* YYO */
    { unclosed "{" 1 "}" 5 }                /* YYO */
  | BANGLBRACE record_expr_gcc error            /* YYO */
    { unclosed "{" 1 "}" 5 }                /* YYO */
  | BANGLBRACE record_expr_f90 error            /* YYO */
    { unclosed "{" 1 "}" 5 }                /* YYO */
  ...
  | BEGIN seq_expr END
      { reloc_exp $2 }
  | BEGIN END
      { mkexp (Pexp_construct (Lident "()", None, false,Keep)) }
...
let_binding:
    val_ident fun_binding
      { ({ppat_desc = Ppat_var $1; ppat_loc = rhs_loc 1; ppat_ext = None}, $2) } /* XXO */
...
core_type2:
  ...
  | SHARP constr_longident core_type2           /* XXO */
      { mktyp(Ptyp_annotation($2, [ $3 ])) }	/* XXO */
;
