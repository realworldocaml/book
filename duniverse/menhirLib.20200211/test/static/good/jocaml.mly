...
/* $Id: jocaml.mly,v 1.1 2005/08/23 11:15:13 fpottier Exp $ */
...
%left  INFIXOP0 EQUAL LESS GREATER   LESSGREATER   /* = < > etc */
%right INFIXOP1                         /* @ ^ etc */
%right COLONCOLON                       /* :: */
...
top_structure:
  ...
  | LET LOC loc_bindings                 { [mkstr (Pstr_location $3)] }
;
use_file:
    use_file_tail                        { $1 }
...
lident_comma_list:
    LIDENT {[$1]}
  | LIDENT COMMA lident_comma_list { $1::$3 }
;
...
def_bindings:
    def_binding                                    { [$1] }
  | def_bindings AND def_binding                { $3 :: $1 }
;
...
join_pattern:
  ...
  | LIDENT PREFIXOP simple_pattern
           ...
           | _ -> syntax_error () }
  | UIDENT simple_pattern   { mkpat(Ppat_construct(Lident $1,Some $2,false))}
  | join_pattern BAR join_pattern      { mkpat(Ppat_or($1,$3)) }
;

def_parameters:
        /* nothing */       {[]}
    | LPAREN lident_comma_list RPAREN { $2 }
;

loc_bindings:
      loc_binding                   { [$1] }
    | loc_binding AND loc_bindings { $1::$3 }
;

loc_init:
       DO process0 { $2 }
    | /* nothing */ { mkexp(Pexp_null) }

loc_binding:
    LIDENT loc_modules loc_defs loc_init { $1, mklistexp $2, $3, $4 }
;

list_of_modules:
       UIDENT   { [ mkexp(Pexp_module $1) ] }
    |  UIDENT SEMI list_of_modules { (mkexp(Pexp_module $1))::$3 }
;

loc_modules:
    /* nothing */ { [] }
  | LBRACKET list_of_modules RBRACKET { $2 }
  | LBRACKET RBRACKET { [] }
;
...
join_match_cases:
    pattern join_match_action                        { [$1, $2] }
  | join_match_cases BAR pattern join_match_action        { ($3, $4) :: $1 }
;
join_match_action:
    MINUSGREATER process0                       { $2 }
  | WHEN seq_expr MINUSGREATER process0         { mkexp(Pexp_when($2, $4)) }
;
...
process0:
  ...
  | LBRACE process RBRACE { $2 }
  | LBRACE process error { unclosed "{" 1 "}" 3 }
  | LBRACE error { syntax_error () }
  | LOC loc_bindings { mkexp(Pexp_location($2, mkexp(Pexp_null))) }
;
...
constructor_declarations_ext:
     constructor_declarations                    { $1 }
   | tag_constructor_declarations                { $1 }
;
tag_constructor_declarations:
    tag_constructor_declaration                     { [$1] }
  | tag_constructor_declarations BAR tag_constructor_declaration { $3 :: $1 }
;

/* Ne pas permettre de creer des types concrets multiples si un tag
    est specifie.  */
...
constructor_declaration:
     constr_ident ext_constructor_arguments   { ($1, None, $2) }
;
...
ext_constructor_arguments:
    /*empty*/                                    { Pconstr_tuple[]}
  | OF core_type_list                            { Pconstr_tuple(List.rev $2)}
  | OF LBRACE label_declarations opt_semi RBRACE { Pconstr_record(List.rev $3)}
;
