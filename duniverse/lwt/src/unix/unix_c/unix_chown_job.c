...
/* Informations:
   ...
   - this is the expected prototype of the C function [chown]:

       int chown(char* path, int ower, int group)

   - these are the expected ocaml externals for this job:

       external chown_job : string -> int -> int -> unit Lwt_unix.job = "lwt_unix_chown_job"
       external chown_sync : string -> int -> int -> unit = "lwt_unix_chown_sync"
*/
...
/* Structure holding informations for calling [chown]. */
struct job_chown {
...
/* The function calling [chown]. */
static void worker_chown(struct job_chown* job)
{
  /* Perform the blocking call. */
  job->result = chown(job->path, job->ower, job->group);
  /* Save the value of errno. */
  job->errno_copy = errno;
}

/* The function building the caml result. */
static value result_chown(struct job_chown* job)
{
  /* Check for errors. */
  if (job->result < 0) {
    ...
    /* Raise the error. */
    unix_error(error, "chown", string_argument);
  }
  /* Free the job structure. */
  lwt_unix_free_job(&job->job);
  /* Return the result. */
...
CAMLprim value lwt_unix_chown_job(value path, value ower, value group)
{
  /* Get the length of the path parameter. */
  mlsize_t len_path = caml_string_length(path) + 1;
  /* Allocate a new job. */
  struct job_chown* job = lwt_unix_new_plus(struct job_chown, len_path);
  ...
  job->job.result = (lwt_unix_job_result)result_chown;
...
CAMLprim value lwt_unix_chown_job(value Unit)
{
  lwt_unix_not_available("chown");
  return Val_unit;
}
