...
let%test_module _ = (module struct
  ...
  let%expect_test "time moves backwards" =
    ...
    let calibrator = Calibrator.Private.create_using ~tsc:!tsc ~time:!time ~samples:[] in
    let calibrate =
  ...
  let test_time_and_cycles samples_file ~error_limit ~alpha ~verbose =
    ...
    List.iter samples ~f:(fun (tsc, time) ->
      ...
      if verbose then
        printf "%f %f %s %f\n%!" cur_error !ewma_error (Int63.to_string (to_int63 tsc)) time;
  ...
  let test_time_and_cycles_nanos samples_file ~error_limit ~alpha ~verbose =
    ...
    List.iter samples ~f:(fun (tsc, time) ->
      ...
      let cur_error =
        scale_us_abs (Int63.to_float (Span.Private.to_int63
                                        (diff
                                           (of_int63 time_nanos)
