  ...
  (struct
    ...
    struct
      ...
      module Intf_common
        ...
        Iobuf_intf.Accessors_common with type ('a, 'b, 'c) t = ('a, 'b, 'c) Intf.t
                                     and type 'a bin_prot = 'a Intf.bin_prot =
      struct
        open Intf
      ...
      module Intf_read
          (Intf : sig
             include Iobuf_intf.Accessors_read
        ...
        with type ('a, 'b, 'c) t = ('a, 'b, 'c) Intf.t
         and type 'a bin_prot := 'a Bin_prot.Type_class.reader =
      struct
        open Intf
        ...
        let%test_unit _ =
          ...
          then begin
            ...
            int_pos_1 8 int64_be_exn (large_int 0x0102 0x0304 0x0506 0x0708)
              "A\001\002\003\004\005\006\007\008J";
            int_pos_1 8 int64_le_exn (large_int 0x090a 0x0b0c 0x0d0e 0x0f10)
              "A\016\015\014\013\012\011\010\009J";
            int_pos_1 8 int64_be_exn (-(large_int 0x0102 0x0304 0x0506 0x0709))
              "A\254\253\252\251\250\249\248\247J";
            int_pos_1 8 int64_le_exn (-(large_int 0x0102 0x0304 0x0506 0x0709))
      ...
      module Intf_write
          (Intf : sig
             include Iobuf_intf.Accessors_write
        ...
        with type ('a, 'b, 'c) t = ('a, 'b, 'c) Intf.t
         and type 'a bin_prot := 'a Bin_prot.Type_class.writer =
      struct
        open Intf
        ...
        let%test_unit _ =
          ...
          if arch_sixtyfour
          then begin
            int_pos_1 4 uint32_be_trunc (large_int 0 0 0xF6F5 0xF4F3)
              "A\246\245\244\243FGHIJ";
            int_pos_1 4 uint32_le_trunc (large_int 0 0 0xFBFA 0xF9F8)
      ...
      let test_peek_to (blito : (Peek.src, _) Base.Blit.blito)
      ...
      let%test_unit _ =
        ...
        if is_safe then
          (assert (is_error (try_with (fun () -> Peek.char t ~pos:(-1))));
           ...
           assert
             (is_error (try_with (fun () -> Poke.char t ~pos:(String.length s) 'z'))))
      ;;
        ...
        (module struct
          ...
          let tests tests =
            List.iter tests ~f:(fun test ->
              ...
              try test (); printf "%S\n" (Iobuf.to_string t)
              with e -> print_s [%sexp (e : exn)])

          let%expect_test "unsigned Pokes of negative ints" =
            tests (List.map Poke.([ uint8_trunc
                                  ; uint16_be_trunc; uint16_le_trunc
                                  ; uint32_be_trunc; uint32_le_trunc
                                  ; uint64_be_trunc; uint64_le_trunc ])
                     ~f:(fun upoke () ->
                       upoke t (-1 lxor 0xFF lor Char.to_int '!') ~pos:0));
            [%expect {|
              "!1234567"
              "\255!234567"
              "!\255234567"
          ...
          let%expect_test "signed Pokes of out-of-range ints" =
            tests (List.map Poke.([ int8_trunc
                                  ; int16_be_trunc; int16_le_trunc
                                  ; int32_be_trunc; int32_le_trunc ])
          ...
          let%expect_test "unsigned Fills don't check sign" =
            tests (List.map Fill.([ uint8_trunc
                                  ; uint16_be_trunc; uint16_le_trunc
                                  ; uint32_be_trunc; uint32_le_trunc
                                  ; uint64_be_trunc; uint64_le_trunc ])
                     ~f:(fun ufill () ->
          ...
          let%expect_test "signed Fills don't check range" =
            tests (List.map Fill.([ int8_trunc
                                  ; int16_be_trunc; int16_le_trunc
                                  ; int32_be_trunc; int32_le_trunc ])
                     ~f:(fun fill () ->
