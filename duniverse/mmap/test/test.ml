...
let tests () =
  ...
  begin
    ...
    let a =
      array1_of_genarray (Mmap.V1.map_file fd float64 c_layout true [|10000|])
    in
    Unix.close fd;
    ...
    let b =
      array2_of_genarray
        (Mmap.V1.map_file fd float64 fortran_layout false [|100; -1|])
    in
    Unix.close fd;
    let ok = ref true in
    for i = 0 to 99 do
    ...
    let fd = Unix.openfile mapped_file [Unix.O_RDONLY] 0 in
    let c =
      array2_of_genarray (Mmap.V1.map_file fd float64 c_layout false [|-1; 100|])
    in
    Unix.close fd;
    let ok = ref true in
    for i = 0 to 99 do
    ...
    let c =
      array2_of_genarray
        (Mmap.V1.map_file fd ~pos:800L float64 c_layout false [|-1; 100|])
    in
    Unix.close fd;
    let ok = ref true in
    for i = 1 to 99 do
    ...
    let c =
      array2_of_genarray
        (Mmap.V1.map_file fd ~pos:79200L float64 c_layout false [|-1; 100|])
    in
    Unix.close fd;
    let ok = ref true in
    for j = 0 to 99 do
    ...
    test 1 true
      begin try
        ignore (Mmap.V1.map_file fd float64 c_layout true [|-1; 100|]); false
      with
