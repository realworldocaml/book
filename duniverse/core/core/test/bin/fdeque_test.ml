...
let lpush l q = List.fold ~init:q ~f:enqueue_back l
...
let test =
  ...
  >::: [ ("basic"
          >:: fun () ->
            ...
            "empty3"
            @? is_empty
                 (drop_front_exn (drop_front_exn (drop_front_exn (drop_front_exn q3))));
            "misc"
            @? (to_list
                  (lpush
                     [ 2; 3 ]
       ...
       ; ("invariants and list bisimulation"
          >:: fun () ->
            ...
            in
            ignore
              (List.fold (List.range 0 1000) ~init:(empty, []) ~f:(fun (q, l) i ->
                 let q, l =
