...
let ( === ) b1 b2 = if b1 <> b2 then failwithf "%s <> %s" (to_string b1) (to_string b2) ()

let same_sexp b1 b2 =
  let s1 = to_string b1
  and s2 = to_string b2 in
  if s1 <> s2 then failwithf "%s <> %s" s1 s2 ()
;;
...
let test =
  ...
  >::: [ "simplification"
         ...
         ; ("if6"
           ...
           ; (***)
           ("ifnot1"
            >:: fun () -> if_ (not_ false_) (base "A") (base "B") === base "A")
         ; ("ifnot2"
            >:: fun () -> if_ (not_ true_) (base "A") (base "B") === base "B")
         ]
       ; "flattening"
         >::: [ ("and"
                 >:: fun () ->
       ...
       ; ("values"
          >:: fun () ->
            ...
            if [ "this"; "other"; "yet another thing" ] <> values
