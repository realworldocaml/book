  ...
  >::: [ ("slice"
       ...
       ; ("lsplit2_exn"
          >:: fun () ->
            ("none"
             @?
             try
               ignore (S.lsplit2_exn str1 ~on:' ' : string * string);
               false
             with
       ...
       ; ("rsplit2_exn"
          >:: fun () ->
            ("none"
             @?
             try
               ignore (S.rsplit2_exn str1 ~on:' ' : string * string);
               false
             with
             | Not_found_s _ | Caml.Not_found -> true
             | _ -> false);
       ...
       ; ("split"
          >:: fun () ->
            ...
            "consecutive_delims" @? (S.split "bocci ball" ~on:'c' = [ "bo"; ""; "i ball" ]))
       ; ("split_on_chars"
          >:: fun () ->
            ...
            "consecutive_delims"
            @? (S.split_on_chars "bocci ball" ~on:[ 'c' ] = [ "bo"; ""; "i ball" ]);
       ...
       ; ("suffix"
          >:: fun () ->
            ...
            let test len res = sprintf "%d" len @? (S.suffix base len = res) in
            test 0 "";
            test 1 "9";
            test 2 "89";
            test 10 base;
       ...
       ; ("prefix"
          >:: fun () ->
            let base = "0123456789" in
            let test len res = sprintf "%d" len @? (S.prefix base len = res) in
            test 0 "";
            test 1 "0";
            test 2 "01";
            test 10 base;
       ...
       ; ("drop_suffix"
          >:: fun () ->
            let base = "0123456789" in
            let test len res = sprintf "%d" len @? (S.drop_suffix base len = res) in
            test 0 base;
            test 1 "012345678";
            test 2 "01234567";
            test 10 "";
       ...
       ; ("drop_prefix"
          >:: fun () ->
            let base = "0123456789" in
            let test len res = sprintf "%d" len @? (S.drop_prefix base len = res) in
            test 0 base;
            test 1 "123456789";
            test 2 "23456789";
            test 10 "";
       ...
       ; "mem"
         >::: [ ("default"
                 >:: fun () ->
                   let test t c b = sprintf "%c in %s = %b" c t b @? (S.mem t c = b) in
                   (let t = "abc" in
                    test t 'a' true;
                    test t 'b' true;
                    test t 'c' true;
