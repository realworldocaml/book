  ...
  >::: [ ("merge1"
       ...
       ; ("sexp"
          >:: fun () ->
            ...
            try
              ignore (Map.t_of_sexp string_of_sexp int_of_sexp s_dup : _ Map.t);
              assert false
            with
            | _ -> ())
       ; ("of_alist"
          >:: fun () ->
            ...
            let m =
              match Map.of_alist a with
              | `Ok x -> x
              | `Duplicate_key _ -> failwith "argh"
            in
            "1" @? (Map.find_exn m "a" = 1 && Map.find_exn m "d" = 4);
            let a_dup = [ "a", 1; "b", 2; "c", 3; "b", 4; "e", 5 ] in
            ("2"
             @?
             match Map.of_alist a_dup with
             | `Ok _ -> false
             | `Duplicate_key x -> x = "b");
            "3" @? (Map.to_alist (Map.of_alist_exn a) = List.sort ~compare:Poly.ascending a);
            try
              ignore (Map.of_alist_exn a_dup : _ Map.t);
              assert false
            with
            | _ -> ())
       ; ("for_all/exists"
