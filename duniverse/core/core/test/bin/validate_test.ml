            ...
            ("inf/nan"
             ...
             let res =
               V.name_list
                 ...
                 ; V.name
                     "nan"
                     (Float.validate_bound ~min:(Incl 0.) ~max:(Incl 100.) Float.nan)
                 ; V.name
                     "inf"
                     (Float.validate_bound ~min:(Incl 0.) ~max:(Incl 100.) Float.infinity)
                 ]
             in
             let expected =
               [ "(bar.nan \"value is NaN\")"; "(bar.inf \"value is infinite\")" ]
             in
             List.sort ~compare:Poly.ascending (V.errors res)
             = List.sort ~compare:Poly.ascending expected);
            ("nesting"
