...
let rec count acc = function
  ...
  | TestList l -> List.fold_left l ~f:count ~init:acc
;;
  ...
  | TestList l ->
    List.fold_left
      ...
      ~f:(fun (pa_ounit, total) test ->
        ...
        | test -> pa_ounit, count total test)
      ~init:(0, 0)
  | v -> 0, count 0 v
;;
  ...
  let cnt_ounit =
    ...
    , Arg.Unit
        (fun () ->
           ...
           exit 0)
    , " Count how many of the tests were converted to pa_ounit (for informational \
       purposes only)" )
  in
  ignore
    (run_test_tt_main ~arg_specs:[ cnt_ounit ] (Test.all ()) : OUnit.test_result list)
;;
