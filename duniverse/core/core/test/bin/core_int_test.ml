...
let rand state =
  let rec aux acc cnt =
    ...
    then acc
      ...
      aux ((2 * acc) + bit) (cnt - 1))
  in
  let aval = aux 1 (Random.State.int state (Sys.word_size_in_bits - 3)) in
  if Random.State.bool state then -aval else aval
;;
...
let test =
  ...
  >::: [ ("to_string_hum"
          >:: fun () ->
            ...
            "random"
            @? (List.init ~f:(fun _ -> rand state) 10_000 |> List.for_all ~f:esc_test);
