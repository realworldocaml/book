...
let () =
  List.iter
    ...
    ~f:(fun (input, expected) ->
      let got =
        try Int_conversions.insert_underscores input with
        | exn -> failwithf "input = %s  exn = %s" input (Exn.to_string exn) ()
      in
      if got <> expected
      then failwithf "input = %s  got = %s  expected = %s" input got expected ())
;;
...
struct
  ...
  let xs =
    let module E = Examples (X) in
    E.examples
  ;;

  let ys =
    let module E = Examples (Y) in
    E.examples
  ;;
