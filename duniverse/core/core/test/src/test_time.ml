...
let%expect_test "Span.to_parts + Span.create" =
  List.iter span_examples ~f:(fun span ->
    ...
    Core.print_s [%sexp (span, parts : Time.Span.t * Time.Span.Parts.t)];
...
let%expect_test "Ofday.to_parts + Ofday.create" =
  List.iter ofday_examples ~f:(fun ofday ->
    ...
    Core.print_s [%sexp (ofday, parts : Time.Ofday.t * Time.Span.Parts.t)];
...
let%expect_test "time zone offset parsing" =
  let test string =
    print_endline (Time.to_string_utc (Time.of_string_with_utc_offset string))
  in
...
let%expect_test "time zone invalid offset parsing" =
  let test here string =
    require_does_raise here (fun () -> Time.of_string_with_utc_offset string)
  in
...
let%expect_test "times with implicit zones" =
  let test f = show_raise (fun () -> print_endline (Time.to_string_utc (f ()))) in
  test (fun () ->
    Time.Stable.With_utc_sexp.V2.t_of_sexp (Sexp.of_string "(2013-10-07 09:30)"));
  [%expect {|
    2013-10-07 09:30:00.000000Z
    "did not raise" |}];
  require_does_raise [%here] (fun () -> Time.of_string_with_utc_offset "2013-10-07 09:30");
  [%expect
    {|
    (time.ml.Make.Time_of_string
     "2013-10-07 09:30"
