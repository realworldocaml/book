...
let of_sorted_array () =
  ...
  ignore (Int.Map.of_sorted_array sorted_array : _ Int.Map.t Or_error.t)
;;
...
let remove =
  ...
  fun () -> ignore (Map.remove map deep_key : _ Map.t)
;;

let old_map_merge t1 t2 ~f =
  let all_keys = List.dedup_and_sort ~compare (List.append (Map.keys t1) (Map.keys t2)) in
  List.fold ~init:Map.Poly.empty all_keys ~f:(fun t key ->
    let z =
...
let gen_diff_test m ~number_of_diff ~diff =
  ...
  let map2 =
    ...
    List.fold (List.init number_of_diff ~f:gen_pair) ~init:map1 ~f:(fun acc (key, data) ->
...
let diff_by_iter2 map1 map2 ~data_equal =
  ...
  Map.iter2 map1 map2 ~f:(fun ~key ~data ->
    ...
    | `Both (v1, v2) ->
      if not (data_equal v1 v2) then results := (key, Some v2) :: !results);
  !results
;;

let command =
  let symmetric_diff t1 t2 ~data_equal =
    Map.symmetric_diff t1 t2 ~data_equal |> Sequence.to_list
  in
  Bench.make_command
    ...
    ; Test.create
        ~name:"Map.symmetric_diff-10"
        (gen_diff_test 100_000 ~number_of_diff:10 ~diff:symmetric_diff)
    ; Test.create
        ~name:"Map.symmetric_diff-100"
        (gen_diff_test 100_000 ~number_of_diff:100 ~diff:symmetric_diff)
    ; Test.create
        ~name:"Map.symmetric_diff-1000"
        (gen_diff_test 100_000 ~number_of_diff:1000 ~diff:symmetric_diff)
    ; Test.create
        ~name:"Map.symmetric_diff_by_iter2-10"
        (gen_diff_test 100_000 ~number_of_diff:10 ~diff:diff_by_iter2)
    ; Test.create
        ~name:"Map.symmetric_diff_by_iter2-100"
        (gen_diff_test 100_000 ~number_of_diff:100 ~diff:diff_by_iter2)
    ; Test.create
        ~name:"Map.symmetric_diff_by_iter2-1000"
