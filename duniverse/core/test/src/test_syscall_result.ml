...
let%test_unit "to_result doesn't allocate" =
  for _ = 1 to 1000 do
    for int = -(Int.Private.length_preallocated_errnos - 1) to Int.Private.length_preallocated_ms - 1
    do
      ...
      [%test_result: (int, Unix.Error.t) Result.t] result
        ~expect:(if is_ok t
                 then Ok int
                 else Error (Unix.Error.of_system_int ~errno:(-int)));
    done;
  done
;;
