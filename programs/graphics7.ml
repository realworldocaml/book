...
object (self : 'self)
   ...
   method draw = Graphics.fill_poly self#get_vertices
   method transform matrix =
      {< vertices = Array.map matrix#transform vertices >}

   val mutable last_vertices = [||]
   val mutable int_vertices = [||]
   method private get_vertices =
      if last_vertices != vertices then begin
         int_vertices <- Array.map int_coord vertices;
...
let image =
   ...
   let add_star (x, y, scale) =
      let trans = (transform#new_translate x y) ** (transform#new_scale scale scale) in
      image#add (star#transform trans) in
   List.iter add_star
      [0.35, 0.50, 0.15;
       0.12, 0.95, 0.12;
       0.35, 0.95, 0.10;
...
(image#transform (transform#new_scale 180.0 180.0))#draw;;
